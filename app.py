...
from simulation import breguet_range_nm, step_simulation, mission_simulation, sensitivity_table, RESERVE_MIN
...
def run():
    try:
        ...
        mission  = mission_simulation(cfg, strategy=strategy)
        breguet  = breguet_range_nm(cfg)

        # Cruise fuel flow at initial MTOW (for gal/hr display)
        P_cruise_kw  = cfg.propfan.shaft_power_w(v, drag_n, rho, sos) / 1000.0
        fuel_flow_gph = cfg.engine.fuel_flow_L_h(P_cruise_kw) / 3.78541

        # Fuel totals based on mission simulation
        range_miles   = mission["range_nm"] * 1.15078
        liters_burned = mission["fuel_burned_kg"] / cfg.engine.fuel_density_kg_l
        gal_burned    = liters_burned / 3.78541
        range_mpg     = range_miles / gal_burned if gal_burned > 0 else 0.0
        trip_cost_usd = gal_burned * 2.50

        # Chart uses full mission log (shows climb + cruise + descent)
        log  = mission.get("log", [])
        ...
        return jsonify({
            ...
            "range": {
                # Mission profile (with 45-min reserve + climb/descent)
                "mission_nm":     round(mission["range_nm"],         0),
                "mission_km":     round(mission["range_km"],         0),
                "endurance_hr":   round(mission["endurance_hr"],     1),
                "avg_ld":         round(mission["avg_ld"],           2),
                "avg_eta":        round(mission["avg_propulsive_efficiency"]*100, 1),
                # Fuel breakdown
                "reserve_kg":     round(mission["reserve_kg"],       1),
                "reserve_min":    RESERVE_MIN,
                "takeoff_fuel_kg": round(mission["takeoff_fuel_kg"], 1),
                "climb_fuel_kg":  round(mission["climb_fuel_kg"],    1),
                "cruise_fuel_kg": round(mission["cruise_fuel_kg"],   1),
                # Max range (no reserves, step sim)
                "max_nm":         round(stepped["range_nm"],         0),
                "max_km":         round(stepped["range_km"],         0),
                # Breguet reference
...
def optimize():
    ...
    and cruise_altitude_ft that maximise Breguet range.  Fixed: fuel load,
    cruise speed, cabin width, stall speed, material, engine type.

    Altitude bounds match the UI slider (2,000–60,000 ft) but can be
    overridden with alt_min_ft / alt_max_ft in the request.
    """
    try:
        data = request.get_json()
        ...
        # ── Coarse grid (4-D: fan × wingspan × power × altitude) ──────
        ...
        # 8 altitudes linearly spaced over the allowed band (rounded to 500 ft)
